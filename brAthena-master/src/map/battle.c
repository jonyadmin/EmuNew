...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
			...
			if( s_bl != t_bl ) {
				if( sd->state.killer ) {
					...
					if (t_bl->type == BL_PC && sd->duel_group == BL_UCCAST(BL_PC, t_bl)->duel_group)
						...
						return 0;
					} else if(map->getcell(s_bl->m,s_bl,s_bl->x, s_bl->y, CELL_CHKPVP) && map->getcell( t_bl->m,t_bl,t_bl->x, t_bl->y, CELL_CHKPVP))
				{	// Addon Cell PVP [Ize]
					 state |= BCT_ENEMY;
				}
			}
	...
	{ //Non pvp/gvg, check party/guild settings.
		if( flag&BCT_PARTY || state&BCT_ENEMY ) {
			int s_party = status->get_party_id(s_bl);
			if(s_party && s_party == status->get_party_id(t_bl) && !(battle_config.cellpvp_party_enable && map->getcell( t_bl->m,t_bl,t_bl->x, t_bl->y, CELL_CHKPVP ))){ // Addon Cell PVP [Ize]
				state |= BCT_PARTY;
			} else {
				state |= BCT_ENEMY;
			}
		...
		if( flag&BCT_GUILD || state&BCT_ENEMY ) {
			...
			//if(s_guild && t_guild && (s_guild == t_guild || (!(flag&BCT_SAMEGUILD) && guild->isallied(s_guild, t_guild))))
				if(s_guild && t_guild && (s_guild == t_guild || (!(flag&BCT_SAMEGUILD) && guild->isallied(s_guild, t_guild))) && !(battle_config.cellpvp_guild_enable && map->getcell( t_bl->m,t_bl,t_bl->x, t_bl->y, CELL_CHKPVP)) ) // Addon Cell PVP [Ize]
				state |= BCT_GUILD;
		}
