...
int unit_walktoxy_timer(int tid, int64 tick, int id, intptr_t data) {
	...
	if(sd) {
		...
		if(!sd->state.pvp && map->getcell( sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP))
			map->pvp_area(sd, 1);
		else if(sd->state.pvp && !map->getcell( sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP))
			map->pvp_area(sd, 0);
...
int unit_walktoxy( struct block_list *bl, short x, short y, int flag)
{
	...
	if (bl->type == BL_PC)
	{
		...
		if (sd && battle_config.cellpvp_walkout_delay > 0 && DIFF_TICK( timer->gettick(), sd->pvpcan_walkout_tick) < battle_config.cellpvp_walkout_delay && !map->getcell( sd->bl.m,&sd->bl,x, y, CELL_CHKPVP ) )
		{
