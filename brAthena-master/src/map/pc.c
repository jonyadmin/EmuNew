...
int pc_makesavestatus(struct map_session_data *sd)
{
	...
	if( ( map->list[sd->bl.m].flag.nosave && sd->state.autotrade != 2 ) || map->list[sd->bl.m].instance_id >= 0 || map->getcell( sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP)) { // Addon Cell PVP [Ize]
...
int pc_isUseitem(struct map_session_data *sd,int n)
{
	...
	switch( nameid ) { // TODO: Is there no better way to handle this, other than hardcoding item IDs?
		...
		case ITEMID_GIANT_FLY_WING:
			...
			if( map->list[sd->bl.m].flag.noteleport || map_flag_gvg2(sd->bl.m) || map->getcell( sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP) ) { // Addon Cell PVP [Ize]
		...
		case ITEMID_SIEGE_TELEPORT_SCROLL:
			...
			if( nameid != ITEMID_WING_OF_FLY && nameid != ITEMID_GIANT_FLY_WING && map->list[sd->bl.m].flag.noreturn || map->getcell( sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP) )	// Addon Cell PVP [Ize]
				return 0;
			break;
...
int pc_setpos(struct map_session_data* sd, unsigned short map_index, int x, int y, clr_type clrtype) {
	...
	if( sd->state.changemap ) { // Misc map-changing settings
		...
		// Addon Cell PVP [Ize]
		if(sd->state.pvp && map->getcell( sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP))
...
int pc_memo(struct map_session_data* sd, int pos) {
	...
	//if( sd->bl.m >= 0 && (map->list[sd->bl.m].flag.nomemo || map->list[sd->bl.m].flag.nowarpto) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE) ) {
		if((sd->bl.m >= 0 && (map->list[sd->bl.m].flag.nomemo || map->list[sd->bl.m].flag.nowarpto) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) || map->getcell( sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP) ) {	// Addon Cell PVP [Ize]
...
void pc_deathmatch(struct map_session_data* sd, clr_type clrtype)
{
	...
	if( map->getcell(sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP) && battle_config.cellpvp_deathmatch ) 
	{
		do {
			x = rand() % (map->list[sd->bl.m].xs - 2) + 1;
			y = rand() % (map->list[sd->bl.m].ys - 2) + 1;
		} while( !map->getcell(sd->bl.m,&sd->bl,x, y, CELL_CHKPVP) || !map->getcell(sd->bl.m,&sd->bl,x, y, CELL_CHKPVP) );
...
int pc_dead(struct map_session_data *sd,struct block_list *src) {
	...
	if( battle_config.bone_drop==2
	 || (battle_config.bone_drop==1 && (map->list[sd->bl.m].flag.pvp || (map->getcell( sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP) && sd->state.pvp))) // Addon Cell PVP [Ize])
	)
	{
	...
	// Addon Cell PVP [Ize]
	if (map->getcell( sd->bl.m,&sd->bl,sd->bl.x, sd->bl.y, CELL_CHKPVP ) && battle_config.cellpvp_deathmatch && sd->state.pvp) {
